    ...
    return (
        <form className='w-full h-full flex flex-col gap-2 xs:gap-4 sm:gap-5' onSubmit={handleSubmit}>
            ...
            <div className='grid gap-1 xs:gap-2 sm:gap-2.5'>
                ...
                <input className='`block h-[3.2rem] lg:h-[4rem] px-5 bg-transparent border rounded-xl border-white text-xs sm:text-sm font-thin focus:bg-transparent focus:outline-none text-white focus:border-gray-300 placeholder:text-off_white placeholder:uppercase' type="email" name='email' required onChange={handleChange} value={formData.email} />
            </div>
            <div className='grid gap-1 xs:gap-2 sm:gap-2.5'>
                <label className='uppercase text-xs font-thin tracking-wide ms-5' htmlFor="">Password</label>
                <input className='`block h-[3.2rem] lg:h-[4rem] px-5 bg-transparent border rounded-xl border-white text-xs sm:text-sm font-thin focus:bg-transparent focus:outline-none text-white focus:border-gray-300 placeholder:text-off_white placeholder:uppercase' type="password" name='password' placeholder='Type your password here...' title='(Atleast 8 characters, uppercase, lowercase, numbers and special characters)' required onChange={handleChange} value={formData.password} />
            </div>
            <div className='grid gap-1 xs:gap-2 sm:gap-2.5'>
                <label className='uppercase text-xs font-thin tracking-wide ms-5' htmlFor=""></label>
                <input className='`block h-[3.2rem] lg:h-[4rem] px-5 bg-transparent border rounded-xl border-white text-xs sm:text-sm font-thin focus:bg-transparent focus:outline-none text-white focus:border-gray-300 placeholder:text-off_white placeholder:uppercase' type="password" name='confirmPassword' placeholder='Retype your password here...' required onChange={handleChange} value={formData.confirmPassword} />
            </div>
            <div>
                {err && <p className='text-white'>Password Don't Match</p>}
            </div>
            <p className='lg:hidden text-xs uppercase font-thin ms-5'>Already have an account? <Link href='/auth/login' className='font-medium underline'>Log In</Link></p>
            ...
            <div className='mt-7 sm:mt-12'>
                ...
                <div className='flex items-center mt-2'>
                    ...
                    <p className='uppercase text-xs sm:text-sm font-thin grow lg:text-center'>Step 0/2 Completed</p>
                </div>
            </div>
